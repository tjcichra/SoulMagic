...
public class SoulArrowEntityRenderer extends EntityRenderer<SoulArrowEntity> {
	...
	public SoulArrowEntityRenderer(EntityRenderDispatcher entityRenderDispatcher_1) {
	...
	public void render(SoulArrowEntity arrow, float float_1, float float_2, MatrixStack matrixStack_1, VertexConsumerProvider vertexConsumerProvider_1, int int_1) {
		matrixStack_1.push();
		matrixStack_1.multiply(Vector3f.POSITIVE_Y.getDegreesQuaternion(MathHelper.lerp(float_2, arrow.prevYaw, arrow.yaw) - 90.0F));
		matrixStack_1.multiply(Vector3f.POSITIVE_Z.getDegreesQuaternion(MathHelper.lerp(float_2, arrow.prevPitch, arrow.pitch)));
		
		float float_12 = (float) arrow.shake - float_2;
		if (float_12 > 0.0F) {
			float float_13 = -MathHelper.sin(float_12 * 3.0F) * float_12;
			matrixStack_1.multiply(Vector3f.POSITIVE_Z.getDegreesQuaternion(float_13));
		}
		...
		matrixStack_1.multiply(Vector3f.POSITIVE_X.getDegreesQuaternion(45.0F));
		...
		for (int int_3 = 0; int_3 < 4; ++int_3) {
			matrixStack_1.multiply(Vector3f.POSITIVE_X.getDegreesQuaternion(90.0F));
